...
export function HoursTable({ locale }: { locale: Locale }) {
  ...
  return (
    <div>
      <p className="tabular min-h-7 font-medium text-cocoa">
        {status ? (
          <span className="inline-flex items-center gap-2">
            <span aria-hidden="true" className={`inline-block h-2.5 w-2.5 rounded-full ${isOpen ? "bg-crust" : "border-2 border-muted"}`} />
            {status}
          </span>
        ) : null}
      </p>
          ...
          {restaurant.hours.map((d) => {
            ...
            return (
              <tr key={d.day} className="border-b border-line">
                <th scope="row" className={`py-2.5 text-left ${isToday ? "font-medium text-cocoa" : "font-normal text-cocoa"}`}>
                  {v.days[d.day]}{" "}
                  {isToday ? <span className="ml-2 rounded-full bg-crust px-2 py-0.5 text-[0.75rem] font-bold text-cocoa">{v.today}</span> : null}
                </th>
                <td className={`tabular py-2.5 text-right ${isToday ? "font-medium text-cocoa" : "text-cocoa"}`}>
