...
import { formatCount, formatPhotoDate, formatRating } from "@/lib/format";
import { restaurant } from "@/lib/restaurant";
import { reviews } from "@/lib/reviews";

/**
 * «Opiniones» (DESIGN.md §7.7). Сводка честная: рейтинг 4,3, вся гистограмма звёзд
 * (включая 30 единиц) и десять тем Google с числом упоминаний. Витрина — отзывы
        ...
        <Reveal className="flex flex-col gap-10">
          <div>
            <p className="flex items-end gap-3">
              <span className="tabular font-display text-[5.2rem] leading-[0.85] text-cocoa">{formatRating(value, locale)}</span>
              <span className="pb-1 text-muted">{r.outOf}</span>
            </p>
            <Stars value={value} idPrefix="reviews" size={20} className="mt-4 text-crust" />
            <p className="tabular mt-2 text-muted">{r.basedOn(formatCount(count, locale))}</p>
          </div>

          <div>
            <h3 className="eyebrow text-cocoa">{r.histogram}</h3>
            <dl className="mt-4 flex flex-col gap-2">
              {histogram.map((h) => (
                <div key={h.stars} className="grid grid-cols-[5rem_1fr_3rem] items-center gap-3 text-[0.9rem]">
                  ...
                  <dd aria-hidden="true" className="h-2 overflow-hidden rounded-full bg-flour">
                    <span className="block h-full rounded-full bg-crust" style={{ width: `${Math.max(1.5, (h.count / max) * 100)}%` }} />
                  </dd>
                  <dd className="tabular text-right text-cocoa">{formatCount(h.count, locale)}</dd>
                </div>
              ))}
            </dl>
          </div>
          ...
          <div>
            <h3 className="eyebrow text-cocoa">{r.topicsTitle}</h3>
            <ul className="mt-4 flex flex-wrap gap-2">
              {restaurant.topics.map((t) => (
                <li key={t.key} className="inline-flex items-center gap-2 rounded-full border border-line bg-flour px-3 py-1.5 text-[0.9rem] text-cocoa">
                  {r.topics[t.key]}
        ...
        <div>
          <RevealGroup as="ul" className="grid gap-4 md:grid-cols-2">
            {reviews[locale].map((rv) => (
              <RevealItem as="li" key={rv.id}>
                <figure className="flex h-full flex-col rounded-2xl border border-line bg-flour p-6 sm:p-7">
                  <Stars value={rv.stars} idPrefix={`rv-${rv.id}`} size={15} className="text-crust" />
                  <span className="sr-only">{r.stars(rv.stars)}</span>
                  <blockquote lang={locale} className="mt-4 flex-1 leading-relaxed whitespace-pre-line text-cocoa">
                    <p>{rv.text}</p>
                  </blockquote>
          ...
          <Reveal className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2">
            <a
              ...
              target="_blank"
              rel="noopener"
              className="inline-flex min-h-11 items-center gap-2 font-bold text-cocoa underline decoration-crust underline-offset-4 hover:decoration-2"
            >
              {r.readAll}
