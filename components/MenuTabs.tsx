 ...
 * `overflow-x-clip`, а не `overflow-hidden` (hidden сломал бы sticky). При смене вкладки
 * страница возвращается к началу ленты, иначе новый список начинается выше экрана.
 *
 * Первая панель приходит из SSR без анимации (motion вписал бы opacity:0 в HTML);
 * кроссфейд — только после первой смены вкладки, и только opacity/transform.
 */
export function MenuTabs({ locale, sections }: { locale: Locale; sections: MenuSection[] }) {
  ...
  const select = (id: string, focus = false) => {
    ...
    if (strip && strip.getBoundingClientRect().top < 0) {
      strip.scrollIntoView({ block: "start", behavior: reduced ? "auto" : "smooth" });
    }
  };
  ...
  return (
    <div>
      <div ref={stripRef} className="sticky top-18 z-20 -mx-5 border-b border-line bg-flour/95 px-5 backdrop-blur-md sm:-mx-8 sm:px-8">
        <div role="tablist" aria-label={m.tabsLabel} className="-mb-px flex gap-1 overflow-x-auto py-2 [scrollbar-width:none]">
          {sections.map((s, i) => {
            ...
            return (
              <button
                ...
                className={`inline-flex min-h-11 shrink-0 cursor-pointer items-center whitespace-nowrap rounded-full px-4 text-[0.95rem] font-bold transition-colors duration-200 ${
                  selected ? "bg-cocoa text-on-dark" : "text-cocoa hover:bg-pine"
                }`}
              >
                {s.title[locale]}
              </button>
      ...
      <div id={`${base}-panel`} role="tabpanel" aria-labelledby={`${base}-tab-${active}`} tabIndex={0} className="pt-10 focus-visible:outline-offset-8">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            key={active}
            initial={changed && !reduced ? { opacity: 0, y: 12 } : false}
            animate={{ opacity: 1, y: 0 }}
            exit={reduced ? { opacity: 1 } : { opacity: 0, y: -8 }}
            transition={{ duration: reduced ? 0 : 0.3, ease: "easeOut" }}
            className="grid gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.3fr)] lg:gap-14"
          >
