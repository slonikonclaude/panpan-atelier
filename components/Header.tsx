...
import { IconClose, IconDirections, IconMenuBars, IconPhone } from "@/components/icons";
import { getDictionary, type Locale } from "@/lib/dictionaries";
import { restaurant } from "@/lib/restaurant";
import { hrefFor } from "@/lib/site";

/**
 * Шапка: мука поверх муки hero, после первых 40 px прокрутки — линия снизу.
 ...
 *
 * Главное действие — «Cómo llegar» (брони нет: кафетерия, где заказывают у стойки —
 * DESIGN.md §2). До 420 px кнопка остаётся одной иконкой, иначе вордмарк, EN, кнопка и
...
export function Header({ locale }: { locale: Locale }) {
  ...
  const links = [
    { href: "#desayunos", label: dict.nav.breakfast },
    { href: "#carta", label: dict.nav.menu },
    { href: "#obrador", label: dict.nav.bakery },
    { href: "#local", label: dict.nav.place },
    { href: "#opiniones", label: dict.nav.reviews },
    { href: "#visitanos", label: dict.nav.visit },
  ];

  const ring = "border-cocoa/25 hover:border-cocoa";

  return (
    <header
      className={`fixed inset-x-0 top-0 z-40 bg-flour/95 text-cocoa backdrop-blur-md transition-colors duration-300 ${
        scrolled || open ? "border-b border-line" : "border-b border-transparent"
      }`}
    >
      <a
        href="#contenido"
        className="sr-only rounded-sm bg-cocoa text-flour focus:not-sr-only focus:absolute focus:left-4 focus:top-3 focus:z-50 focus:px-4 focus:py-2"
      >
      ...
      <div className="mx-auto flex h-18 max-w-7xl items-center justify-between gap-3 px-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-7 xl:flex" aria-label={dict.nav.sections}>
          {links.map((l) => (
            <a key={l.href} href={l.href} className="inline-flex min-h-11 items-center text-[0.95rem] transition-colors duration-200 hover:text-crust-deep">
              {l.label}
            </a>
          ))}
        </nav>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            className="inline-flex h-11 min-w-11 items-center justify-center gap-2 rounded-full bg-crust px-3 text-[0.95rem] font-bold text-cocoa transition-colors duration-200 hover:bg-crust-soft min-[420px]:px-5"
          >
            <IconDirections width={18} height={18} />
            <span className="sr-only min-[420px]:not-sr-only">{dict.cta.directions}</span>
      ...
      <div id={panelId} hidden={!open} className="h-[calc(100dvh-4.5rem)] overflow-y-auto bg-flour text-cocoa xl:hidden">
        <nav className="mx-auto max-w-7xl px-5 py-4 sm:px-8" aria-label={dict.nav.sections}>
          <ul>
            {links.map((l) => (
              <li key={l.href}>
                <a href={l.href} onClick={() => setOpen(false)} className="flex min-h-14 items-center border-b border-line font-display text-[1.8rem] text-cocoa">
                  {l.label}
                </a>
              </li>
            ))}
          ...
          <div className="flex flex-col gap-3 pt-6 sm:flex-row">
            <a
              ...
              target="_blank"
              rel="noopener"
              className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-crust px-5 text-base font-bold text-cocoa"
            >
              <IconDirections width={18} height={18} />
              {dict.cta.directions}
              <span className="sr-only"> {dict.cta.newTab}</span>
            ...
            <a
              ...
              className="tabular flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full border border-cocoa/30 px-5 text-base font-bold text-cocoa"
            >
