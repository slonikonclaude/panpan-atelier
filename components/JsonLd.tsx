...
/**
 * schema.org: CafeOrCoffeeShop + Bakery (кафетерия с обрадором). Без aggregateRating:
 * рейтинг Google на собственном сайте заведения — «self-serving review», Google такую
 * разметку не принимает. Часы — из карточки Google (DESIGN.md §1).
 */
...
export function JsonLd({ locale }: { locale: Locale }) {
  ...
  const data = {
    ...
    image: [abs(`/photos/${photos.hero.name}-1600.webp`), abs(`/photos/${photos.vitrinaBolleria.name}-1600.webp`), abs(`/photos/${photos.fachada.name}-1600.webp`)],
    telephone: restaurant.phone.tel,
    priceRange: `${restaurant.pricePerPerson.from}–${restaurant.pricePerPerson.to} €`,
    servesCuisine: locale === "es" ? ["Desayunos", "Bollería", "Cafetería"] : ["Breakfast", "Pastries", "Coffee"],
