 ...
 *   _photos/logo-white.png → public/brand/logo-480.webp и -960.webp (маска, с альфой)
 *
...
const BRAND_OUT = path.join("public", "brand");
...
if (existsSync("_photos/logo-white.png")) {
  for (const w of [480, 960]) {
    await sharp("_photos/logo-white.png")
      .resize({ width: w })
      .webp({ quality: 92, alphaQuality: 100 })
      .toFile(path.join(BRAND_OUT, `logo-${w}.webp`));
  }
}
