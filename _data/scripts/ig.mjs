...
const PORT = 9854;
fs.mkdirSync(`${SCRATCH}/ig`, { recursive: true });
await launch({ port: PORT, profile: "chrome-profile-ig", headless: true });
const c = await attach(PORT);
await c.send("Page.enable");
await c.navigate("https://www.instagram.com/panpanatelier/", 10000);
...
save(`${SCRATCH}/ig/profile.json`, prof);
...
for (const link of prof.links) {
  ...
  save(`${SCRATCH}/ig/${code}.json`, { link, ...r });
