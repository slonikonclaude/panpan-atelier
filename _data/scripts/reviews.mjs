...
const PORT = Number(process.env.PORT || 9852);
const URL_R = `https://www.google.com/maps/place/PanPan+Atelier/@39.4674636,-0.3685427,17z/data=!3m1!4b1!4m6!3m5!1s0xd6048b680fb2875:0x5a50d7ac5ee24336!8m2!3d39.4674636!4d-0.3685427!16s%2Fg%2F1tj5s1rt?hl=${hl}`;
...
if (!/PanPan/i.test(name)) { console.log("WRONG PLACE"); process.exit(1); }

// Если список отзывов не открылся — нажать вкладку «Reseñas»/«Reviews».
{
  await c.evaluate(`[...document.querySelectorAll('button[role=tab]')].find(b=>/Reseñas|Reviews/.test(b.innerText||b.getAttribute('aria-label')))?.click()`);
  await sleep(3500);
}
console.log("has sort button:", await c.evaluate(`!![...document.querySelectorAll('button')].find(b=>/Ordenar|Sort/.test(b.getAttribute('aria-label')||b.innerText))`));
if (sort === "newest") {
...
save(`../maps/reviews-${hl}-${sort}-full.json`, reviews);
